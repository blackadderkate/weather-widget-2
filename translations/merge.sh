...
if [ -f "po/template.pot" ]; then
	newPotDate=`grep "POT-Creation-Date:" template.pot.new | sed 's/.\{3\}$//'`
	oldPotDate=`grep "POT-Creation-Date:" po/template.pot | sed 's/.\{3\}$//'`
	sed -i 's/'"${newPotDate}"'/'"${oldPotDate}"'/' "template.pot.new"
	changes=`diff "po/template.pot" "template.pot.new"`
	if [ ! -z "$changes" ]; then
...
echo "msgfmt --desktop --template=\"$DIR/template.desktop\" -d \"$DIR/po/\" -o \"$DIR/new.desktop\""
msgfmt --desktop --template="$DIR/template.desktop" -d "$DIR/po/" -o "$DIR/new.desktop"
